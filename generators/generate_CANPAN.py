...
if args.version == "5a":
    ...
    for sw in range(1, switches, 2):
        switchPairItems.append(
            {
                "displayTitle": f"Switches {sw} & {sw + 1}",
...
for sw in range(1, switches+1):
    switchOptions.append({"label": f"Switch {sw}", "value": sw})
if args.version == "5a":
...
if args.version != "5a":
    ...
    for sw in range(1, switches + 1):
        selfSodEventOptions.append({"label": f"Switch {sw}", "value": sw})
